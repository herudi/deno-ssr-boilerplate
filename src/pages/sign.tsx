...
function Sign({ message }: PageProps) {
  return (
    <div>
      <Helmet>
        <title>Login</title>
      ...
      >
        <div class={tw`max-w-sm w-full space-y-8`}>
          <div>
            <h2
              class={tw`mt-6 text-center text-3xl font-extrabold text-gray-900`}
            >
              Sign Page
            </h2>
            ...
            {message && (
              ...
              >
                ...
                <span class={tw`block sm:inline`}>{message}</span>
...
Sign.methods = ["GET", "POST"] as string[];

Sign.initProps = async (rev: RequestEvent) => {
